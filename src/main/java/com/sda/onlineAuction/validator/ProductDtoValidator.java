...
public class ProductDtoValidator {
    ...
    public void validate(ProductDto productDto, BindingResult bindingResult) {
        ...
        try {
            ...
            if (priceAsInteger <= 0) {
                FieldError fieldError = new FieldError("productDto", "startBiddingPrice", "Price must be positive!");
                bindingResult.addError(fieldError);
            }
        } catch (NumberFormatException numberFormatException) {
            FieldError fieldError = new FieldError("productDto", "startBiddingPrice", "Price must be a number!");
            bindingResult.addError(fieldError);
        }
